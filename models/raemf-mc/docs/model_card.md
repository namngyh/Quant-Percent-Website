...
### 4.8 Còn thiếu để vận hành — ĐÃ GIẢI QUYẾT (2026-08-26)

Cả ba mảnh dưới đây từng không tồn tại. Nay đã có; xem
`docs/realtime_bayes/STATUS-4-models.md`.

- ~~**Lưu/tải mô hình.**~~ `raemf_mc/persistence.py` lưu posterior biến phân kèm
  fingerprint dữ liệu và hằng số centering. Đo thật: `predict` từ bundle đã lưu
  mất **6,8 giây** (500 path, horizon 20) — đúng bậc mà mục này ước lượng về lý
  thuyết, thay cho "tối thiểu ~9 phút" khi chưa tách được fit khỏi predict.
  `assert_history_unchanged` từ chối dự báo nếu lịch sử đã bị sửa so với lúc fit.
- ~~**CLI/orchestration.**~~ `python -m raemf_mc.cli discover-source | fit | predict`.
- ~~**Đường nạp dữ liệu sống.**~~ `raemf_mc/data/source.py` đọc read-only từ
  TimescaleDB (`default_transaction_read_only=on`), đi qua **đúng** tầng validate
  của đường CSV (`drop_exact_duplicates`, `validate_ohlc_invariants`,
  `check_implausible_daily_moves`).

**Lưu ý về dữ liệu:** đối chiếu database với CSV đã phát hiện một bug parse
volume trong `ingest._reconstruct_number_fields` — file export bỏ số 0 đầu của
mỗi nhóm hàng nghìn, và hàm này pad zero cho 4 trường giá nhưng không pad cho
volume, làm **3879/6264 phiên** sai gấp 10–1000 lần. Đã sửa và có test. Mọi số
liệu trong model card này được tính **trước** khi sửa; volume có vào đặc trưng
nào thì con số đó cần chạy lại.

Việc còn lại: fit quy mô nghiên cứu (8 seed, ~9,5 giờ) chưa chạy lại trên dữ
liệu đã sửa và đã kéo dài tới hiện tại.
