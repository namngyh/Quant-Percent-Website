...
def _reconstruct_number_fields(parts: list[str]) -> tuple[float, float, float, float, int]:
    ...
    # Volume is thousands-separated across fields exactly like the prices, and
    # the export strips leading zeros from every group after the first the same
    # way: 538,080,668 arrives as ("538", "80", "640"). Plain concatenation
    # gives 53_880_640 -- a tenth of the real figure, and still a plausible
    # VN-Index turnover, so nothing downstream would flag it. Verified against
    # the database on the 6264 overlapping sessions: padding each trailing group
    # back to 3 digits reconciles every one of the 3879 that previously
    # disagreed, to within float32 rounding of the source file.
    volume = int(volume_parts[0] + "".join(part.rjust(3, "0") for part in volume_parts[1:]))
