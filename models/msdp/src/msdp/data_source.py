"""Read-only market data sources for the online tier.

The batch pipeline reads a static CSV via :mod:`msdp.data_io`. The online tier
polls whatever store the sessions actually land in, so it needs a driver-agnostic
interface plus connectors that *cannot* modify the source:
...
* PostgreSQL/TimescaleDB is opened with ``default_transaction_read_only=on`` so
  the *server* refuses writes, whatever the client asks for;
* the CSV backend only ever calls read APIs.

Ported from the RAFF repository (`vnindex_model/data_source.py`), which proved
the interface against the same VN-Index series. The two models share a data
source, so keeping the shape identical means one confirmed `data.source` block
works for both.
"""
...
def _normalize(
    frame: pd.DataFrame, column_map: dict[str, str], date_unit: str | None
) -> pd.DataFrame:
...
def _slice_since(
    frame: pd.DataFrame, since: pd.Timestamp | None, lookback_buffer_days: int | None
) -> pd.DataFrame:
    if since is None:
...
def resolve_dsn(config: dict[str, Any]) -> str:
    ...
    machine's ``.env`` and never in a tracked YAML file. There is deliberately no
    fallback to ``data.source.path``: that key points at the batch CSV, and
...
class PostgresMarketDataSource:
    ...
    windowing identical to the file backends instead of re-deriving the
...
class CsvMarketDataSource:
    """Local file source reusing the defensive parser in :mod:`msdp.data_io`.

    Kept as the default so the online tier is exercisable before a database
    ...
    def _read(self) -> pd.DataFrame:
        frame = load_market_data(self.path)
...
def build_market_data_source(config: dict[str, Any]) -> MarketDataSource:
    ...
    MySQL is absent on purpose: no such store exists here, and guessing a schema
    would violate the repository's rule against silent assumptions.
    """
